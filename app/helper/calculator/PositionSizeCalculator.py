...
class PositionSizeCalculator:
    ...
    def calculate_crypto_trade_size(self, entry_price, stop_loss_price):
        """
    ...
    def calculate_indices_trade_size(self, entry_price, stop_loss_price, point_value=50):
        """
    ...
    def calculate_forex_trade_size_jpy(self, entry_price, stop_loss_price, lot_size=100000):
        """
        ...
        return self.calculate_forex_trade_size_non_jpy(entry_price, stop_loss_price, pip_value=pip_value)

    def calculate_forex_trade_size_non_jpy(self, entry_price, stop_loss_price, pip_value=10):
        """
    ...
    def calculate_commodity_position_size(self, entry_price, stop_loss_price, contract_size,
                                          tick_size):
        risk_amount = (self._risk_manager.max_risk_percentage / 100) * self._risk_manager.account_balance
    ...
    def calculate_qty(self, asset_class: str, order: Order) -> float:
        moneyatrisk = self._risk_manager.calculate_money_at_risk()
        order.money_at_risk = moneyatrisk
        qty = 0.00
        if asset_class == AssetClassEnum.CRYPTO.value:
            qty = self.calculate_crypto_trade_size(float(order.price), float(order.stopLoss))
        if asset_class == AssetClassEnum.FX.value:
            qty = self.calculate_forex_trade_size_non_jpy(float(order.price), float(order.stopLoss))
