...
from app.helper.calculator.entry.ratio.BaseRatio import BaseRatio
from app.helper.calculator.entry.ratio.FixedRatio import FixedRatio
            ...
            for ratio in rangeRatio:
                try:
                    ...
                    if self.is_condition_full_filled(profit, stop, entry, direction):
                        estimatedProfits.append(ProfitStopEntry(profit=profit, stop=stop, entry=entry))
        ...
        try:
            for ratio in rangeRatio:
                try:
                    ...
                    logger.debug("values: {},{},{},{}".format(profit, stop,entry,ratio))
                    if self.is_condition_full_filled(profit, stop, entry, direction):
                        estimatedStops.append(ProfitStopEntry(profit=profit, stop=stop, entry=entry))
                except Exception as e:
        ...
        try:
            for ratio in rangeRatio:
                try:
                    ...
                    logger.debug("values: {},{},{},{}".format(profit, stop,entry,ratio))
                    if self.is_condition_full_filled(profit, stop, entry, direction):
                        estimatedEntries.append(ProfitStopEntry(profit=profit, stop=stop, entry=entry))
                except Exception as e:
