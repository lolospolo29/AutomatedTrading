...
class PDRiskCalculator:
    ...
    def _calculateStop(self, pdArray:PDArray, orderDirection:OrderDirection, riskMode:RiskMode) -> float:
        if pdArray.name == "BPR":
    ...
    def calculateStopsSpecific(self,pdArrays:list[PDArray],orderDirection:OrderDirection,riskMode:RiskMode) -> list[float]:
        ...
        for pdArray in pdArrays:
            stops.append(self._calculateStop(pdArray, orderDirection, riskMode))
        return stops
    ...
    def _calculateEntry(self, pdArray: PDArray, orderDirection: OrderDirection, riskMode: RiskMode) -> float:
        if pdArray.name == "BPR":
    ...
    list[float]:
        stops = []
        for pdArray in pdArrays:
            stops.append(self._calculateEntry(pdArray, orderDirection, riskMode))
        return stops
