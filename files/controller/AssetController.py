...
class AssetController:
    ...
    def get_assets(self) -> list[dict]:
        assets = self._AssetManager.return_assets()
        dict_assets = []
        for asset in assets:
            try:
                ...
                self._logger.error(
                    "Error appending asset to list Name: {name},Error:{e}".format(name=asset._name, e=e))
                continue
        return dict_assets
    ...
    def get_asset_classes(self):
        asset_classes: list[AssetClass] = self._AssetManager.return_asset_classes()
        dict_asset_classes = []
